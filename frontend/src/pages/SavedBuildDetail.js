...
const SavedBuildDetail = () => {
  ...
  const handleExportCsv = async () => {
    try {
      const rows = [];
      let total = 0;
      // Use already-fetched component details
      for (const comp of components) {
        const price = Number(comp.price || comp.cost || 0);
        total += price;
        rows.push({
          Type: comp.type || '',
          Name: comp.name || comp.model || '',
          Manufacturer: comp.manufacturer || comp.brand || '',
          Model: comp.model || '',
          Price: price,
      ...
      const meta = {
        ...
        TotalPrice: build.totalPrice != null ? build.totalPrice : total,
        ComputedPrice: Number(computedPrice || total),
        TotalWattage: build.totalWattage || computedWattage || ''
      };
      const timestamp = new Date();
      const pad = (n) => String(n).padStart(2, '0');
      const fnameTs = `${timestamp.getFullYear()}-${pad(timestamp.getMonth()+1)}-${pad(timestamp.getDate())}-${pad(timestamp.getHours())}${pad(timestamp.getMinutes())}`;
      const filename = `${(build.name || 'Build').replace(/\s+/g,'')}_${fnameTs}.csv`;

      // Build CSV content
      const escape = (v) => typeof v === 'string' ? `"${v.replace(/"/g,'""')}"` : v;
      const headerMeta = Object.keys(meta).join(',');
      const valuesMeta = Object.values(meta).map(escape).join(',');
      const header = ['Type','Name','Manufacturer','Model','Price','Wattage'].join(',');
      const lines = rows.map(r => [r.Type, r.Name, r.Manufacturer, r.Model, r.Price, r.Wattage]
        .map(escape)
        .join(',')
      );
      const csv = [headerMeta, valuesMeta, '', header, ...lines].join('\n');
      // Detect native Android and use Capacitor Filesystem + Share
      const platform = typeof Capacitor.getPlatform === 'function' ? Capacitor.getPlatform() : 'web';
      const isNative = typeof Capacitor.isNativePlatform === 'function' ? Capacitor.isNativePlatform() : (Capacitor.isNative || false);
      const isAndroidNative = isNative && platform === 'android';

      if (isAndroidNative) {
        const path = `BuildGenie/${filename}`;
        try {
          ...
          try {
            await Share.share({
              title: 'Exported Build CSV',
              text: 'BuildGenie export',
              url: uriRes.uri,
            });
          } catch {
            // If share fails, at least notify saved path
            alert(`CSV saved to Documents/BuildGenie as ${filename}`);
          }
        } catch (fsErr) {
          console.error('Capacitor write/share failed, falling back', fsErr);
          // Fallback to web download behavior
